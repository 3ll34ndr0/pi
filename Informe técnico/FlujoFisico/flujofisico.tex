...
\includegraphics[scale=.7]{figuras/CeldEstandarAlto.eps}
  \caption{Grilla de interconexionado y riel de alimentación de las celdas estándar}
...
Partimos desde la descripción estructural\footnote{El resultado de la síntesis hecha con \textbf{lava} es un \emph{netlist} a nivel de compuerta, listo para ser usado por una herramienta de \gls{pnr}.} que producimos en el capítulo \ref{diseñoDigital}. De \emph{Electric} usaremos la herramienta llamada \textbf{\emph{Silicon Compiler}}, que se encarga de ubicar y conectar las celdas según el \emph{netlist} VHDL. En la figura \ref{fig:SCconf} vemos la configuración necesaria para hacer el \gls{pnr} con nuestras celdas estándar. La configuración se realiza para ajustar la herramienta a las reglas de \gls(drc) y las dimensiones de nuestra celdas estándar. Pero la variable de ajuste para variar el \emph{floorplan} es \emph{Number of rows of cells}. Según la cantidad de filas que asignemos, será el resultado obtenido. Como regla de oro para todo \emph{layout} es que sea lo más cuadrado posible, ya que de esta forma es más eficiente el uso del área, cuando integramos nuestro circuito con otros de mayor jerarquía. La métrica de selección del resultado será el área que ocupe nuestro circuito, y la relación entre sus lados: cuanto más pequeño y cuadrado mejor. En las tablas \ref{tabla:rippleCarry}, \ref{tabla:skalansky} y \ref{tabla:brent-kung} vemos los resultados \gls(pnr) para todos los sumadores analizados, con 3 tamaños distintos y para diferentes alternativas de \emph{floorplan}, variando el parámetro \textbf{\emph{Number of rows of cells}}.
...
\item Es necesario agregar las celdas estandar que serán utilizadas en el circuito, donde comienza la descripción estructural del VHDL.
\item Los nombres no pueden usar los símbolos [ y ], por lo cual es necesario eliminarlos.
\end{enumerate} 
 
Estas tareas se realizan automáticamente si modificamos el código del programa de \textbf{lava} llamado \verb|VhdlNew.hs|, encargado de crear el \emph{netlist}, y con un script escrito en \textbf{perl} que mostramos en el apéndice \ref{scriptPerl}.

\subsection{Distintas alternativas}
A modo de ejemplo ilustrativo, mostramos el \emph{layout} de los 3 sumadores para el tamaño de 8 bits.
...
\toprule
\textbf{Skalansky} & \multicolumn{3}{c}{8}       & \multicolumn{4}{c}{16}                & \multicolumn{3}{c}{32}      \\ \midrule
