...
-- Circuitos síncronos
--addSeq :: (Signal Bool,Signal Bool) -> Signal Bool
addSeq (a, b) = sum
	where
...
--dilay :: Generic a => a -> a 
dilay :: Signal Bool -> Signal Bool
dilay xs = dly xs

addSq (a, b) = sum
	where
	  carryIn		= dly carryOut
	  (sum, carryOut)	= fullAdd (carryIn, (a, b))
...
testadder' n = writeVhdlInputOutputNoClk "adder2" adder'
	        (var "cin", [(var "a", var "b")])
                (varList n "sum", var "cout")
test7 n = writeVhdlInputOutputNoClk "BrentKungFastAdder" bKungFastAdder
          (varList n "a", varList n "b")
          (varList n "s", var "cout")

test8 = writeVhdlInputOutputNoClk "dotOp" dotOp
	((var "g1", var "p1"),(var "g",  var "p"))
	(var "go", var "po")


test9 = writeVhdlInputOutput "dotOpClk" dotOp
	((var "g1", var "p1"),(var "g",  var "p"))
	(var "go", var "po")

test10 = writeVhdlInputOutputNoClk "nandinv2" nandinv2
		(var "a", var "b")
		(var "y")

test11 = writeVhdlInputOutputNoClk "inv" inv


--bKungFastAdder = gAndPs ->- fork ->- (evens -|- bKung) ->- dropP ->- sums
test12 n = writeVhdlInputOutputNoClk "sums" sums
          (varList n "a", varList n "b")
          (varList n "sum", var "cout")

test13 n = writeVhdlInputOutputNoClk "gAndPs" gAndPs
          (varList n "a", varList n "b")
          (varList n "gAndP" )

bKunge = evens -|- bKung

test14 n = writeVhdlInputOutputNoClk "bKunge" bKunge
          (varList n "a", varList n "b")
          (varList n "outa", (varList n "outb" ))


test14' n = writeVhdlInputOutputNoClk "bKung" bKung
          (varList n "in")
          (varList n "out")


fork2dropP = fork ->- (evens -|- bKung) ->- dropP 
test15 n = writeVhdlInputOutputNoClk "fork2dropP" fork2dropP
          (varList n "a" )
          ((varList n "outa"),(varList n "outb"))
-- Sincronos
--{-
tests1  = writeVhdlInputOutput "addSeq" addSeq
	(var "a", var "b")
	(var "sum")

tests2  = writeVhdlInputOutput "addSq" addSq
	(var "a", var "b")
	(var "sum")

tests3 = simulateSq edge [high, low, low, high]
tests4 = simulate edge (high)

{-
tests4 n = writeVhdlInputOutput "dilay" dilay
	(varList n "in")
	(varList n "out")
-}
tests5 = simulateSq dilay [low,high]
