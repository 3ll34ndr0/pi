...
opsBool =
  Ops { equalSymbol = \x y     -> equalBool (Signal x) (Signal y)
      ...
      , dlySymbol = \x         -> unSignal $ dlyBool (Signal x)
...
opsInt =
  Ops { equalSymbol = \x y     -> equalInt (Signal x) (Signal y)
      ...
      , dlySymbol = \x         -> unSignal $ dlyInt (Signal x)
...
dly :: Generic a => a -> a
dly x = construct (dl (struct x))
 where
  dl (Object a)                   = Object (dlySymbol (ops a) a)
  dl (Compound as)                = Compound (map dl as)
  dl _                            = wrong Error.IncompatibleStructures
...
varList n s = [ var (s ++ "[" ++  show i ++ "]") | i <- [0..(n-1)] ]
