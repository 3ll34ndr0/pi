...
----------------------------------------------------------------
-- simulate
-- New version of simulateSeq: simulateSq
--simulateSq :: (Generic a, Generic b) => (a -> b) -> [a] -> [b]
simulateSq :: Generic a => (a -> a) -> [a] -> [a]
simulateSq circ []   = []
simulateSq circ inps = runST (
  do roots <- newSTRef []
...
--hago' :: [a] -> [a]
hago' []   = []
hago' (x:xs) = (head ( map  dly [x])):hago' xs

input :: Generic a => [a] -> a
--input xs = foldr1 dly xs
--hago :: Generic a => [a] -> a
input (x:xs) = head algo
  where
   algo = hago' (x:xs)
-- where
--  out = foldr1 dly xs
