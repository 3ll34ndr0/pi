...
-- Esta versión toma en cuenta que el XOR no es realmente necesario ya que por la forma en que está conectado nunca tendrá 2 unos en la entrada.
-- Es una versión mas optima
fullAdd (carryIn, (a, b)) = (sum, carryOut)
    where
       (sum1,carry1) = halfAdd (a, b)
       (sum, carry2) = halfAdd (carryIn, sum1)
       carryOut = or2 (carry2, carry1)
...
test7 n = writeVhdlInputOutputNoClk "BrentKungFastAdder32" bKungFastAdder
          (varList n "a", varList n "b")
          (varList n "sum", var "cout")

test8 n = writeVhdlInputOutputNoClk "Skalansky32" skalansky'
          (varList n "a", varList n "b")
          (varList n "sum", var "cout")
...
--
skalansky :: [(Bit, Bit)] -> ([Bit], Bit)
skalansky abs = (ss,cout)
  where
    gps            = map gpC abs
...
--
skalansky' :: ([Bit], [Bit]) -> ([Bit], Bit)
skalansky' (as,bs) = skalansky (zip as bs)
...
--
buf (gin,pin) = (gin, pin)
-- dop toma un operador y un buffer y hace una funcion de una lista de duplas a lista de duplas.
-- Esto es lo que vamos a usar como red de prefijo cuando tenemos dos entradas
dop [a,b] = [a, dotOp(a,b)]
--
